...
class LoginActivity : AppCompatActivity() {
        ...
        binding.let {
            it.btnLogin.setOnClickListener { view ->
                ...
                if (username.isNotEmpty() && password.isNotEmpty()) {
                    ...
                    Toast.makeText(
                        this@LoginActivity,
                        R.string.login_please_field_data,
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
    ...
    fun login(username: String, password: String) {
        ...
        dialog.show()
        NetworkManager.loginOk(
            username,
            password,
            object : NetworkManager.Companion.NetworkLoginLisener<LoginResponseModel> {
                    ...
                    runOnUiThread {
                        ...
                        if (FileUtils.fileIsExists(this@LoginActivity)) {
                            ...
                            if (isNewBusinessDate(transactionModel)) {
                                var maxEndDate = 0L
                                transactionModel.shift_data.filter { it.end_date.isNotEmpty() }
                                    .forEach {
                                    ...
                                    if (shift.end_date.isNotEmpty()) {
                                        ...
                                        if (endDate == maxEndDate) {
                                            if (shift.close_date) {
                                                ...
                                                val oldBusiness =
                                                    TimeUtil.getDate(shift.business_date)
                            ...
                            if (response.shift.date == null) {
                                ...
                                val objectShift = Shift(
                                    business_date = sdf.format(systemDate),
                                    shift = "1",
                                    start_date = TimeUtil.getStartDate(Date()),
                                    end_date = "",
                                    close_date = false
                                ...
                                val objectShift = Shift(
                                    business_date = response.shift.date,
...
//                    FileUtils.saveNewBusiness(this@LoginActivity, objectShift)
                ...
                override fun onError(errorModel: NetworkErrorModel) {
                    runOnUiThread {
                        ...
                        Toast.makeText(
                            this@LoginActivity,
                            R.string.username_password_incorrect,
                            Toast.LENGTH_LONG
                        ).show()
                    }
                }
    ...
    fun getMaster() {
        ...
        NetworkManager.getDataMasterOk(object :
            NetworkManager.Companion.NetworkLisener<RMDataModel> {
